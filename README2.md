...
- Configurable disk size plus sensible defaults for CPU type, ballooning, BIOS, machine type, and disk options
...
Memory defaults to 2048 MB and disk size defaults to 20 GB. The installer imports the base image, then resizes the VM disk in the selected storage; cached source images stay unchanged. VM IDs must be numeric, at least 100, and unused. Names allow letters, digits, `.`, `_`, and `-`.
